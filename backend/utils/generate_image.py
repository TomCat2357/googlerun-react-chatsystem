...
def generate_image(
    prompt: str, model_name : str, number_of_images = 1, aspect_ratio = '1:1', language = 'auto', add_watermark=True, safety_filter_level = 'block_medium_and_above', person_generation = 'allow_adult'):
    # Vertex AI の初期化（認証情報はGOOGLE_APPLICATION_CREDENTIALSで指定されたファイルから取得）
    vertexai.init(project=os.environ.get("VERTEX_PROJECT") , location=os.environ.get("VERTEX_LOCATION")  )
