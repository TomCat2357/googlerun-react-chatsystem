...
from utils.common import MODELS, GCP_PROJECT_ID, GCP_REGION
        ...
        if isinstance(content, str):
            ...
            if role == "user" and msg.get("files"):
                ...
                for file in msg.get("files", []):
                    ...
                    if mime_type.startswith("image/") or mime_type.startswith("audio/"):
                        ...
                        logger.debug(
                            f"{mime_type}ファイルをバイナリデータとして処理: {file_name}"
                        )
                        ...
                        logger.debug(
                            f"{mime_type}ファイルをVertexAIに送信するpartsに追加しました"
                        )
                    else:
                        # テキスト系ファイルは内容をテキストとして追加
                        logger.debug(
                            f"テキストファイルをテキストデータとして処理: {file_name}"
                        )
                        file_part = Part.from_text(
                            f"\n--- {file_name} ---\n{file_data}\n--- ファイル終了 ---\n"
                        )
                        parts.append(file_part)
            ...
            for part in parts:
                if hasattr(part, "text") and part.text:
                    ...
                    log_parts.append(
                        f"テキスト: {part.text[:20]}{'...' if len(part.text) > 20 else ''}"
                    )
            ...
            for part in content:
                if part.get("type") == "text":
                    ...
                    if image_url.startswith("data:"):
                        ...
                        parts.append(
                            Part.from_data(mime_type=mime_type, data=base64_data)
                        )
...
def common_message_function(
    *, model: str, messages: List[Dict[str, Any]], stream: bool = False, **kwargs
    ...
    try:
        ...
        logger.debug(
            f"MODELS : {model_options}",
        )
        if model not in model_options:
            logger.error(
                f"指定されたモデル '{model}' は許可されていません。許可モデル: {model_options}"
            )
        ...
        if stream:
            ...
            response = gen_model.generate_content(
                content_list, generation_config=generation_config
            )
